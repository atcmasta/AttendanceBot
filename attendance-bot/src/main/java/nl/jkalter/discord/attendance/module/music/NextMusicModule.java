...
public class NextMusicModule implements ICommandModule, IMessageCreateEventListener {
    private static final Logger LOGGER = LoggerFactory.getLogger(NextMusicModule.class);

    private final Command command;
    private final AudioModuleManager manager;

    public NextMusicModule(AudioModuleManager manager) {
        this.manager = manager;
        command = new Command(CommandName.NEXT, "");
    }
    ...
    public void onMessageReceivedEvent(IMessageCreateEventFacade event) {
        ...
        if (command.isAuthorizedRole(event)) {
            ...
            event.sendMessage(String.format("Playing next item for you %s", authorName));
        } else {
